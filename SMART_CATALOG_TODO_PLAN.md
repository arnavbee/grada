...
Status refresh: aligned with current repo state on 2026-03-24
...
- [ ] Implement pattern detection and generation (e.g., Brand + Category + Year + Serial). _(basic template-based generation is implemented; advanced pattern logic pending)_
...
- [ ] Implement auto-fix algorithms (e.g., fill missings with defaults, auto-increment dupes).
      Note: marketplace export generation already performs inline validation checks; the standalone validation endpoint and dedicated review UI are still pending.
...
- [x] Add export history log for generated marketplace exports.
- [ ] Implement `catalog_versions` database tables to store snapshots.
- [ ] Add version dropdown history log for saved catalog snapshots.
...
- [x] Enable marketplace export formats beyond Styli, including Myntra, Amazon, and Ajio.
- [ ] Finish Styli-specific export parity and formatting.
